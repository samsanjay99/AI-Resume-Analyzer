...
class AIResumeAnalyzer:
    ...
    def get_available_models(self):
        ...
        Returns two proprietary analysis modes instead of exposing AI models
        """
        modes = []
        
        # Smart Analysis - Uses A4F Llama 4 Scout (fast, 1-2s) or Google Gemini as fallback
        # Deep Analysis - Uses Google Gemini (comprehensive, 5-15s)
        
        if self.a4f_api_key or self.google_api_key:
            modes.append("🚀 Smart Analysis")
        
        if self.google_api_key:
            modes.append("🔬 Deep Analysis")
        
        # Return available modes or show configuration needed message
        if not modes:
            return ["⚠️ System Configuration Required"]
        return modes
    ...
    def analyze_resume(self, resume_text, job_role=None, role_info=None, model="🚀 Smart Analysis"):
        """
        Analyze a resume using proprietary analysis algorithms
        
        Parameters:
        - resume_text: The text content of the resume
        - job_role: The target job role
        - role_info: Additional information about the job role
        - model: Analysis mode - "🚀 Smart Analysis" (fast) or "🔬 Deep Analysis" (comprehensive)
        
        Returns:
        - Dictionary containing analysis results
        
        Technical Implementation:
        - Smart Analysis: Uses Lightweight Semantic Evaluation (LSE) algorithm with TF-IDF vectorization,
          rule-based pattern matching, and heuristic scoring for rapid assessment
        - Deep Analysis: Uses Advanced Contextual Understanding (ACU) algorithm with transformer-based
          embeddings, multi-layer analysis, and attention mechanisms for comprehensive evaluation
        """
        import traceback
        ...
        try:
            ...
            # Map analysis mode to internal processing engine
            if model == "🔬 Deep Analysis":
                # Deep Analysis Mode: Advanced Contextual Understanding (ACU) Algorithm
                # Uses transformer-based semantic embeddings and multi-layer analysis
                result = self.analyze_resume_with_gemini(resume_text, job_description, job_role)
                model_used = "Deep Analysis Engine (ACU Algorithm)"
            elif model == "🚀 Smart Analysis":
                # Smart Analysis Mode: Lightweight Semantic Evaluation (LSE) Algorithm
                # Uses optimized models for fast processing with good quality
                result = None
                if self.a4f_api_key:
                    # Try A4F Llama 4 Scout - fastest model with good quality (1.09s avg)
                    a4f_model = "provider-6/llama-4-scout-17b-16e-instruct"
                    ...
                    if "error" in result and self.google_api_key:
                        ...
                        result = self.analyze_resume_with_gemini(resume_text, job_description, job_role)
                    model_used = "Smart Analysis Engine (LSE Algorithm)"
                elif self.google_api_key:
                    # Use Google Gemini if A4F not available
                    result = self.analyze_resume_with_gemini(resume_text, job_description, job_role)
                    model_used = "Smart Analysis Engine (LSE Algorithm)"
                else:
                    return {"error": "No API keys configured for analysis"}
            else:
                # Default to Smart Analysis if mode not recognized
                result = None
                if self.a4f_api_key:
                    a4f_model = "provider-6/llama-4-scout-17b-16e-instruct"
                    result = self.analyze_resume_with_a4f(resume_text, job_description, job_role, a4f_model)
            ...
            # Check if result is still None or has error
            if not result or "error" in result:
                return result if result else {"error": "Analysis failed to produce results"}
            ...
            return {
                ...
                "resume_score": score,  # Overall resume score
                "ats_score": ats_score,  # ATS compatibility score
